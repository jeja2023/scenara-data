...
数据平台前端位于 `frontend/data-console`，当前工作台版本为 `0.1.9`。页面包括总览、数据集、版本治理、难例导入和运维探针，面向用户的页面文本、状态标签、错误提示和设置表单均使用中文；协议字段、资源 ID、模型 ID、媒体类型和路径仍保留其机器可读形式。工作台令牌只保存在当前浏览器会话中。开发时先在仓库根目录启动后端，再在前端目录启动 Vite：
