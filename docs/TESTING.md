...
当前门禁覆盖领域状态机、不可变对象引用、RFC 3339 探针、身份/权限、租户隔离、幂等、数据集发布、质量、Outbox、迁移、仓库必需文件、正式契约版本/摘要锁定、领域标注模式/示例校验，以及禁止跨仓库源码导入。`0.1.6` 增加 OCR、Behavior、Fashion 标注语义和多领域难例回归。
